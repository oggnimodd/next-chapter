...
const tabs = [
  {
    label: "Details",
    to: { path: `/book/${id}/details` },
    value: "/book/:id/details",
    component: BookDetails,
  },
  {
    label: "Notes",
    to: { path: `/book/${id}/notes` },
    value: "/book/:id/notes",
    component: BookNotes,
  },
  {
    label: "Review",
    to: { path: `/book/${id}/review` },
...
const currentTabIndex = ref(
  tabs.findIndex(({ value }) => currentTab.value === value)
);
      ...
      <div className="flex flex-col relative">
        ...
        <div className="sticky top-0 bg-background-default z-50">
          <VTabs v-model="currentTabIndex">
            <VTab v-for="tab in tabs" :key="tab.value" :to="tab.to">{{
              tab.label
            }}</VTab>
          </VTabs>
        </div>
        ...
        <div className="mt-4">
          <div v-for="tab in tabs" :key="tab.value">
            <component
              v-if="currentTabIndex === tabs.indexOf(tab)"
              :is="tab.component"
            />
          </div>
        </div>
