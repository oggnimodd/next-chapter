...
const formattedDate = computed(() =>
  review.value
    ? review.value.updatedAt.toLocaleDateString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
        day: "2-digit",
...
<template>
  ...
  <div v-else>
    ...
    <!-- Normal mode -->
    <VCard
      v-if="review && !isEditMode"
      class="mt-2 p-4 rounded-md flex flex-column gap-2"
    >
      <!-- Rating display -->
      <div class="flex gap-x-2 items-center flex-wrap">
        <VRating
          ...
          color="warning"
          class="self-start"
          v-model="review.rating as number"
          readonly
          half-increments
          length="5"
        >
      ...
      <!-- Review content -->
      <VCardTitle class="font-weight-bold primary--text">{{
        review.description
      }}</VCardTitle>
