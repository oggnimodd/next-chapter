...
const isHomePage = route.path === "/dashboard";
...
<template>
  <div :class="clsx(
    ...
    <router-link :to="`/book/${book.id}/details`" class="w-full">
      <img :src="book.cover || './no_cover.png'" :alt="book.title" :class="clsx('w-full mb-3 object-cover w-full', {
        'h-[285px]': isHomePage,
    ...
    <div class="flex justify-between items-center">
      ...
      <div :class="clsx(
        ...
        ">
        <!-- Menu trigger -->
        <VBtn variant="plain" class="rounded-full !min-w-0 !w-10 !h-10" color="primary"
          :id="`menu-card-shelf-${book.id}`">
          <VIcon icon="mdi-dots-horizontal" />
        </VBtn>
        <VBottomSheet v-if="isMobile()" :activator="`#menu-card-shelf-${book.id}`">
          <VCard>
            <VList dense>
        ...
        <VMenu v-else :activator="`#menu-card-shelf-${book.id}`" v-model="menuOpened">
          <VList dense>
            <VListItem @click="moveModalHandlers.open">
    ...
    <!-- Modals -->
    <ModalRemoveBook :opened="removeModalHandlers.state.value" :handlers="removeModalHandlers" :bookId="book.id"
      :title="book.title" />
    <ModalAddToShelf :opened="moveModalHandlers.state.value" :handlers="moveModalHandlers" action="MOVE" :bookId="book.id"
      :title="book.title" />
  </div>
</template>
